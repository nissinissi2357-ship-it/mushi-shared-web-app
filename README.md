...
7. 既存環境に追加ポイント機能を入れる場合は `supabase/point-entries-migration.sql` を実行する
8. 必要に応じて `supabase/security-hardening.sql` を実行する
9. 最初の Admin を作る場合は `supabase/admin-bootstrap.sql` を実行する
10. サンプルを入れたい場合は `supabase/seed.sql` を実行する
11. `npm run dev` で起動する
